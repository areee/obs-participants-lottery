...
function script_description()
    return
        "Lotteri av mötesdeltagare i slumpmässig ordning.\n\nGjord 2021 av Arttu Ylhävuori.\nBaserat på OBS-nedräkningsskript från Jim."
end

function script_properties()
    local props = Obs.obs_properties_create()

    local p = Obs.obs_properties_add_list(props, "source", "Textkälla",
    ...
    Obs.obs_properties_add_text(props, "participants_text", "Deltagarna",
                                Obs.OBS_TEXT_DEFAULT)
    Obs.obs_properties_add_button(props, "start_again_button", "Börja igen",
                                  Start_again_button_clicked)
    Obs.obs_properties_add_int(props, "items", "Namn per rad", 1, 100000, 1)
...
function script_defaults(settings)
    Obs.obs_data_set_default_string(settings, "participants_text",
                                    "Mikael, Katarina, Tommy, Julia, Magnus, Karin, Thomas, Johanna, Rolf, Ann")
