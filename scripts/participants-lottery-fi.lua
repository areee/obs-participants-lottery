...
function script_description()
    return
        "Arvo palaveriin osallistujat satunnaiseen järjestykseen.\n\nTehnyt 2021 Arttu Ylhävuori.\nPerustuu OBS:n ajastinskriptiin, jonka on tehnyt Jim."
end

function script_properties()
    local props = Obs.obs_properties_create()

    local p = Obs.obs_properties_add_list(props, "source", "Tekstin lähde",
    ...
    Obs.obs_properties_add_text(props, "participants_text", "Osallistujat",
                                Obs.OBS_TEXT_DEFAULT)
    Obs.obs_properties_add_button(props, "start_again_button", "Arvo uudelleen",
                                  Start_again_button_clicked)
    Obs.obs_properties_add_int(props, "items", "Nimiä per rivi", 1, 100000, 1)
...
function script_defaults(settings)
    Obs.obs_data_set_default_string(settings, "participants_text",
                                    "Mikko, Katariina, Timo, Julia, Matti, Kiira, Taneli, Juuli, Risto, Niina")
